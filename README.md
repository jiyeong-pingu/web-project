...
1. 클라우드 인프라 구축
   ...
   AWS EC2 프리티어 가상 서버 빌렸음
   
2. 백엔드 로직 개발
 
   Python과 Flask를 이용해서 로그인, 회원가입, 세션 관리, 로그인 실패 시 로그인 화면 돌아가기 기능 등을    구현함
...
5. 소스코드 관리 및 배포
   ...
   터미널에서 코드를 GitHub에 업로드하고 과정 문서화함.

## Challenging part & Solution
LLM으로 대충 웹 구현을 한 뒤에 기능들이 돌아가는게 마음에 들지 않아서(아이디나 비밀번호가 틀렸을 때 다
시 로그인 화면으로 돌아가는게 아닌 로그인 실패 페이지로 갔었음) 파이썬 코드들을 몇 번 바꿨는데 그럴 때마
다 indentation을 틀리고 터미널에서 수정하면 방향키로만 움직여서 수정하는게 많이 불편했었음.
그리고 터미널에서 상위파일과 하위파일을 왔다갔다하고 상위파일에서 명령어를 입력해야하는데 하위파일에서
입력하는 경우가 좀 많았어서 꽤 머리가 아팠음. 해결방법은..그냥 LLM이랑 대화하면서 계속 수정했음.
